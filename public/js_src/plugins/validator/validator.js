...
$.fn.validator = function(options, formats) {
    ...
    // 绑定检验事件
    $inputs.off('validate').on('validate', function() {
    ...
    $form.off('submit').on('submit', function(event) {
        ...
        var $errors = $inputs.filter('.' + config.errclass),
            ...
            noError = config.beforeSubmit.call(form, event) && $errors.length === 0 && !config.isError.call(form);
        ...
        return noError && config.submit.call(form, event);
    });
